CREATE DEFINER=`root`@`%` PROCEDURE `pr_get_kosummary_valuebase`(
...
-- select * from recon_tmp_tkosumm1;
  ...
  */
  ...
  -- return result
  select a.`Row Labels`,sum(a.`Dr Count` + a.`Cr Count`) as Count,sum(a.`Dr Value` + a.`Cr Value`) as `Value`,
  (select fn_get_currency_format(sum(a.`Dr Value` + a.`Cr Value`),'INR',in_conversion_type)) as `Formal Value`,
